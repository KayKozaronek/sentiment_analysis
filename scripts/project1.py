...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    return np.max([0, 1 - label * (np.dot(theta, feature_vector) + theta_0)])
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    losses = 1 - labels *(np.dot(feature_matrix, theta) + theta_0)
    total_hinge_loss = 0
    for loss in losses:
        total_hinge_loss += np.max([0, loss])

    mean_hinge_loss = total_hinge_loss/ len(losses)
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # print(f"feature mat: {feature_matrix}")
            # print(f"Vector of feature mat: {feature_matrix[i]}")
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in order:
            ...
            tmp_theta, tmp_theta_0 = perceptron_single_step_update(
                                        feature_vector, label, tmp_theta, tmp_theta_0)
            theta += tmp_theta
            theta_0 += tmp_theta_0

    avg_theta = theta / (T* len(order))
    avg_theta_0 = theta_0 / (T* len(order))
